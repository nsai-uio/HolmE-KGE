...
class KGOptimizer(object):
    ...
    def __init__(
            self, model, regularizer, optimizer, batch_size, neg_sample_size, double_neg,
             adver_neg_sampling, adversarial_temperature, verbose=True):
        """Inits KGOptimizer."""
        self.model = model
        ...
        self.adver_neg_sampling = adver_neg_sampling
    ...
    def get_neg_samples(self, input_batch):
        ...
        Returns:
            negative_batch: torch.Tensor of shape (neg_sample_size*batch_Size x 3) with negative examples
        """
    ...
    def neg_sampling_loss(self, input_batch):
        ...
        # TODO: add adversarial neg sampling
        if(self.adver_neg_sampling):
            negative_score = F.softmax(negative_score * self.adversarial_temperature, dim = 1).detach() * F.logsigmoid(-negative_score)
            # print(negative_score.shape)
            # print('adversarial neg sample')
        else:
    ...
    def self_neg_sampling_CE_loss(self, input_batch):
        """Compute KG embedding loss with negative sampling.
        ...
        neg_samples = self.get_neg_samples(input_batch)
        negative_score, _ = self.model(neg_samples)
        negative_score = negative_score.view(input_batch.shape[0], -1)
        scores = torch.cat([positive_score, negative_score], dim=1)
        label = torch.tensor([0], dtype=torch.long).cuda()
        labels = label.repeat(input_batch.shape[0]) 
        # assert False
    ...
    def calculate_loss(self, input_batch):
        ...
        if self.neg_sample_size > 0:
            ...
            # loss, factors = self.self_neg_sampling_CE_loss(input_batch) # CrossEntropy loss
        else:
            predictions, factors = self.model(input_batch, eval_mode=True)
            truth = input_batch[:, 2]
