...
class ComplEx(BaseC):
    ...
    def get_queries_2rel(self, lhs_e, queries):
        ...
        lhs_e = lhs_e[:, :self.rank], lhs_e[:, self.rank:]
        _, rel_e, _ = self.get_complex_embeddings(queries)
        lhs_e = torch.cat([
            lhs_e[0] * rel_e[0] - lhs_e[1] * rel_e[1],
            lhs_e[0] * rel_e[1] + lhs_e[1] * rel_e[0]
        ], 1)
        return lhs_e, self.bh(queries[:, 0])
...
class RotatE(BaseC):
    ...
    def get_queries_2rel(self, lhs_e, queries):
        ...
        lhs_e = lhs_e[:, :self.rank], lhs_e[:, self.rank:]
        lhs_e = torch.cat([
            lhs_e[0] * cos - lhs_e[1] * sin,
            lhs_e[0] * sin + lhs_e[1] * cos
        ], 1)
