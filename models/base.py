...
class KGModel(nn.Module, ABC):
    ...
    def get_ranking(self, queries, filters, batch_size=1000):
        ...
        with torch.no_grad():
            ...
            while b_begin < len(queries):
                ...
                # print(scores[range(len(scores)), these_queries[:, 2]].view(1, -1).tolist())
                # print(targets.view(1, -1).tolist())
                assert torch.sum((scores[range(len(scores)), these_queries[:, 2]].view(1, -1)-targets.view(1, -1))**2).item() <1e-1, 'difference between scores in target position and targets'

                # set filtered and true scores to -1e6 to be ignored
                for i, query in enumerate(these_queries):
                    filter_key = (query[0].item(), query[1].item())
                    filter_out = filters[filter_key] 
    ...
    def get_ranking2(self, queries, filters, batch_size=1000):
        """Compute filtered ranking of correct entity for evaluation.
        ...
        with torch.no_grad():
            ...
            while b_begin < len(queries):
                ...
                for i, query in enumerate(these_queries):
                    ...
                    filter_out += [queries[b_begin + i, 2].item()]
                    print(scores[:, queries[b_begin + i, 2]])
                    print(targets)
                    assert False
                    assert scores[:, queries[b_begin + i, 2]] == targets, 'difference between scores in target position and targets is {}'.format((scores[:, queries[b_begin + i, 2]]-targets).tolist())
                    scores[i, torch.LongTensor(filter_out)] = -1e6
                ranks[b_begin:b_begin + batch_size] += torch.sum(
                    (scores >= targets).float(), dim=1
                ).cpu()
                ...
                scores_numpy = np.array(scores[12,:].cpu())
                # scores.to_numpy()
                targets_numpy = np.array(targets[12,:].cpu())

                np.savetxt('score.txt', scores_numpy)
                np.savetxt( 'targets.txt', targets_numpy)
                # targets_numpy.savetxt('targets.txt')

                # stats_file = os.path.join('score.txt')
                # with open(stats_file, "a") as f:
                #     f.write('scores: {}'.format(scores[0,:].tolist()))
                #     f.write('\n\n-------------\n-------------\n-------------\n-------------\n')
                #     f.write('targets: {}'.format(targets.tolist()))
                assert False
            
        return ranks

    def compute_metrics(self, examples, filters, batch_size=50):
